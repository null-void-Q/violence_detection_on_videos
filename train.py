...
def main(trainDataPath,validDataPath,classList,freeze_layers,batchSize,
    ...
    checkpointPath= './checkpoints/model_'+('_adam' if adam  else '_sgd')+str(learning_rate)+('_aug' if augment else '')+'_freeze'+str(freeze_layers)+'-{epoch:02d}-{val_acc:.2f}.hdf5'
    ...
    res = model.fit_generator(trainDataGenerator, epochs=epochs, 
                            ...
                            shuffle=False)
    writeJsontoFile('./history/training_history'+('_adam' if adam else '_sgd')+str(learning_rate)+('_aug' if augment else '')+'_freeze'+str(freeze_layers)+'.json',res.history)
